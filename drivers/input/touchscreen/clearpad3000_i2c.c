/* linux/drivers/input/touchscreen/clearpad3000_i2c.c
 *
...
#define SYNAPTICS_CLEARPAD_NAME			"clearpad3000"
...
#define SYNAPTICS_RETRY_NUM_OF_INITIAL_CHECK	10
...
#define REPORTING_MODE_CONTINUOUS			0x00
#define INTERRUPT_ENABLE_ABS				0x04
...
do {								\
	dev_dbg(&this->i2c->dev, LEVEL "%s: %d: " X,		\
...
struct synaptics_clearpad {
	...
	struct clearpad3000_platform_data *pdata;
	struct mutex lock;
	struct work_struct work;
	struct synaptics_device_info device_info;
	bool device_info_validity; /* false: force flashing */
	struct synaptics_funcarea *funcarea;
...
static void print_buffer(const char *head, const u8 *buf, size_t size)
{
#ifdef DEBUG
	int i;
	char s[3 * 16 + 1];
	...
	for (i = 0; i < size; i++) {
		snprintf(&s[3 * (i%16)], 3, "%02x ", buf[i]);
		if ((i & 0x0f) == 0x0f || i == size - 1)
			printk(KERN_INFO SYNAPTICS_CLEARPAD_NAME
			       "%s [%02x] %s\n",
			       head ? head : "", i & ~0x0f, s);
	}
#endif
...
static int regs_read(struct synaptics_clearpad *this, u16 reg, u8 *buf, int len)
{
	s32 rc;
	...
	for (off = 0; off < len; off += rsize) {
		if (len < off + I2C_SMBUS_BLOCK_MAX)
			rsize = len - off;
		rc = i2c_smbus_read_i2c_block_data(this->i2c, reg + off,
						   rsize, &buf[off]);
		if (rc < 0) {
			dev_err(&this->i2c->dev,
			       "read i2c error 0x%x(%d bytes)\n", reg, len);
			return rc;
		}
	}
	print_buffer("READ", buf, len);
	return 0;
}

static int reg_write(struct synaptics_clearpad *this, u16 reg, u8 val)
{
	s32 rc;
	...
	if (rc)
		dev_err(&this->i2c->dev,
				"write i2c error 0x%x\n", reg);
	print_buffer("PUT", &val, 1);
	return rc;
}

static int regs_write(struct synaptics_clearpad *this, u16 reg,
		const u8 *buf, int len)
{
	int wsize = I2C_SMBUS_BLOCK_MAX;
	int off;
	s32 rc = 0;

	for (off = 0; off < len; off += wsize) {
		if (len < off + I2C_SMBUS_BLOCK_MAX)
			wsize = len - off;
		rc = i2c_smbus_write_i2c_block_data(this->i2c, reg + off,
						   wsize, &buf[off]);
		if (rc < 0) {
			dev_err(&this->i2c->dev,
			       "write i2c error 0x%x(%d bytes)\n", reg, len);
			return rc;
		}
	}
	print_buffer("WRITE", buf, len);
	return rc;
}
...
static int synaptics_clearpad_read_extents(struct synaptics_clearpad *this)
{
	...
	dev_info(&this->i2c->dev, "number of fingers=%d\n",
...
static void synaptics_firmware_reset(struct synaptics_clearpad *this)
{
	...
	dev_info(&this->i2c->dev, "firmware image has been reset\n");
}
...
static int synaptics_clearpad_initialize(struct synaptics_clearpad *this)
{
	...
	rc = synaptics_read(this, SYNF(F01_RMI, QUERY, 0x00), buf, 21);
	if (rc)
		return rc;

	if (this->state != SYN_STATE_RUNNING) {
		dev_info(&this->i2c->dev,
			"device mid %d, prop %d, family %d, fw rev %d\n",
	...
	if (this->flash.image) {
		if (!this->device_info_validity || this->flash.force) {
			/* force firmware update */
			dev_info(&this->i2c->dev, "force firmware update\n");
			this->task = SYN_TASK_FLASH;
			rc = synaptics_clearpad_flash(this);
			return rc;
		}
		if (memcmp(this->flash.product_id,
				info->product_id, HEADER_PRODUCT_ID_SIZE) == 0
		    && this->flash.customer_family == info->customer_family
		    && this->flash.firmware_revision
			    > info->firmware_revision) {
			/* start firmware update */
			dev_info(&this->i2c->dev, "firmware update started\n");
			this->task = SYN_TASK_FLASH;
			rc = synaptics_clearpad_flash(this);
			return rc;
		} else {
	...
	if (this->pdt[SYN_F11_2D].number != SYN_F11_2D_VALUE) {
		...
		this->device_info_validity = false;
		goto running;
	} else
		this->device_info_validity = true;

	/* set reporting mode to continuous while finger present */
	rc = synaptics_put(this, SYNF(F11_2D, CTRL, 0x00),
...
running:
	...
	snprintf(this->result_info, sizeof(this->result_info),
		...
		this->device_info_validity ? "status OK" : "status NG",
		this->flash_requested ? "fw updated" : "no fw update");
	this->flash_requested = false;

	/* notify end of task */
	dev_info(&this->i2c->dev, "result: %s", this->result_info);
...
static int synaptics_flash_program(struct synaptics_clearpad *this)
{
	...
	dev_info(&this->i2c->dev, "flashing enabled\n");
	...
	dev_info(&this->i2c->dev, "firmware erasing\n");
	dev_info(&this->i2c->dev, "flashing data\n");
...
static int synaptics_firmware_check(struct synaptics_clearpad *this)
{
	const u8 *data;
	struct synaptics_flash_image *f = &this->flash;
	struct synaptics_device_info *info = &this->device_info;
	...
	if (this->flash.force) {
		dev_info(&this->i2c->dev, "force flash\n");
	} else if (this->device_info_validity) {
		if (memcmp(this->flash.product_id,
			   info->product_id, HEADER_PRODUCT_ID_SIZE) != 0) {
			dev_err(&this->i2c->dev,
				"firmware has illegal product id '%s'\n",
				make_string(this->flash.product_id,
					HEADER_PRODUCT_ID_SIZE));
			goto error;
		}
		if (this->flash.customer_family != info->customer_family) {
			dev_err(&this->i2c->dev,
			       "firmware has illegal product family %d\n",
	...
	f->data.pos = 0;
	dev_info(&this->i2c->dev, "DATA: length=%d blocks=%d data=%p\n",
	...
	f->config.pos = 0;
	dev_info(&this->i2c->dev, "CONFIG: length=%d blocks=%d data=%p\n",
...
static int synaptics_clearpad_set_power(struct synaptics_clearpad *this)
{
	...
	if (this->state == SYN_STATE_DISABLED) {
		dev_err(&this->i2c->dev, "state == SYN_STATE_DISABLED\n");
		rc = -ENODEV;
		goto err_unlock;
	}
	should_wake = !(active & SYN_STANDBY) && this->input->users;

	if (should_wake && !(active & SYN_ACTIVE_POWER)) {

		dev_info(&this->i2c->dev, "power ON\n");
		msleep(100);
		...
		dev_info(&this->i2c->dev, "power OFF\n");
		...
		dev_info(&this->i2c->dev, "no change (%d)\n", should_wake);
	}
err_unlock:
	UNLOCK(this);
	return rc;
...
static void synaptics_funcarea_initialize(struct synaptics_clearpad *this)
{
	...
	if (funcarea == NULL) {
		dev_info(&this->i2c->dev, "no funcarea\n");
		return;
	}
	...
	for (; funcarea->func != SYN_FUNCAREA_END; funcarea++) {
		...
		dev_info(&this->i2c->dev, "funcarea '%s' [%d, %d, %d, %d]\n",
				   ...
				   int id, int x, int y, int wx, int wy, int z)
{
	...
	if (!pointer->cur.down) {
		pointer->cur.down = true;
		pointer->funcarea
			= synaptics_funcarea_search(this, &pointer->cur);
		if (pointer->funcarea == NULL) {
			LOG_EVENT(this, "no funcarea (%d, %d)\n", x, y);
			pointer->cur.down = false;
			return 0;
		}
	}
				 ...
				 struct synaptics_pointer *pointer)
{
	...
	case SYN_FUNCAREA_BUTTON:
		...
		funcarea = synaptics_funcarea_search(this, &pointer->cur);
		if (funcarea == pointer->funcarea) {
			LOG_EVENT(this, "clicked\n");
			button = (struct synaptics_button *)funcarea->data;
			button->up_count++;
		}
		break;
...
static void synaptics_clearpad_worker(struct work_struct *work)
{
	...
	if ((interrupt & IRQ_SET_POWER)) {
		...
		if (status & 1) {
			dev_info(&this->i2c->dev, "device reset\n");
			/* OSDD - keeping lock??? no need unlock?? */
		...
		char *buf, loff_t pos, size_t size)
{
	...
	if (!this->flash.image) {
		...
		if (size < HEADER_SIZE) {
			dev_err(&this->i2c->dev, "invalid firmware size");
			return -EINVAL;
		}
					    ...
					    bool force)
{
	...
	dev_info(&this->i2c->dev,
		"%s flash\n", force ? "forced" : "conditional");
	memset(this->result_info, 0, SYNAPTICS_STRING_LENGTH);
	this->flash_requested = true;
	this->flash.force = force;

	rc = synaptics_firmware_check(this);
	if (rc)
		goto error;
...
error:
	...
	this->flash_requested = false;
	synaptics_firmware_reset(this);
	dev_info(&this->i2c->dev, "not started: %s", this->result_info);
	UNLOCK(this);
	return rc;
}
...
static int synaptics_clearpad_command_sync(struct synaptics_clearpad *this)
{
	enum   synaptics_state state;

	return wait_event_interruptible(this->task_none_wq,
			synaptics_clearpad_check_task(this, &state, false));
}
					      ...
					      char *buf)
{
	...
	if (!strcmp(attr->attr.name, __stringify(fwinfo)))
		snprintf(buf, PAGE_SIZE,
			"%s, family %d, fw rev %d, task=%s, state=%s\n",
			make_string(this->device_info.product_id,
				HEADER_PRODUCT_ID_SIZE),
			this->device_info.customer_family,
			this->device_info.firmware_revision,
...
static ssize_t synaptics_clearpad_fwflush_show(struct device *dev,
					      struct device_attribute *attr,
					      char *buf)
{
	struct synaptics_clearpad *this = dev_get_drvdata(dev);
	...
	rc = synaptics_clearpad_command_sync(this);
	if (rc) {
		dev_err(&this->i2c->dev, "sync failed\n");
		return rc;
	}

	strncpy(buf, this->result_info, SYNAPTICS_STRING_LENGTH + 1);

	dev_info(&this->i2c->dev, "fwflush: %s\n", buf);
					       ...
					       const char *buf, size_t size)
{
	...
	dev_info(&this->i2c->dev, "flush command: %s\n", buf);
	...
	} else if (strncmp(buf, flush_commands_str[SYN_FLUSH],
		strlen(flush_commands_str[SYN_FLUSH])) == 0) {
		rc = synaptics_clearpad_command_fw_flash(this, 0);
	} else if (strncmp(buf, flush_commands_str[SYN_FORCE_FLUSH],
		strlen(flush_commands_str[SYN_FORCE_FLUSH])) == 0) {
		rc = synaptics_clearpad_command_fw_flash(this, 1);
	} else {
		dev_err(&this->i2c->dev, "illegal command\n");
		rc = -EINVAL;
	}
	if (rc)
		dev_err(&this->i2c->dev, "%s failed\n", __func__);
	return strlen(buf);
}
...
static DEVICE_ATTR(fwrevision, 0600, synaptics_clearpad_state_show, 0);
...
static DEVICE_ATTR(fwflush, 0600,
		   synaptics_clearpad_fwflush_show,
		   synaptics_clearpad_fwflush_store);
...
static int synaptics_clearpad_input_init(struct synaptics_clearpad *this)
{
	...
	this->input->name = SYNAPTICS_CLEARPAD_NAME;
	...
	dev_info(&this->i2c->dev, "Touch area [%d, %d, %d, %d]\n",
...
static int synaptics_clearpad_pm_suspend(struct device *dev)
{
	...
	struct synaptics_clearpad *this = i2c_get_clientdata(client);
	int rc = 0;
	bool go_suspend;
...
static void synaptics_clearpad_early_suspend(struct early_suspend *handler)
{
	...
	dev_info(&this->i2c->dev, "early suspend\n");
	synaptics_clearpad_pm_suspend(&this->i2c->dev);
}
...
static void synaptics_clearpad_late_resume(struct early_suspend *handler)
{
	...
	dev_info(&this->i2c->dev, "late resume\n");
	synaptics_clearpad_pm_resume(&this->i2c->dev);
}
#endif

static int __devinit synaptics_clearpad_probe_i2c(struct i2c_client *i2c,
		const struct i2c_device_id *id)
{
	...
	if (rc) {
		dev_err(&this->i2c->dev,
		       "failed initialization (bad device?)\n");
	...
	LOCK(this);
	rc = request_irq(this->i2c->irq, &synaptics_clearpad_irq,
...
static int __devexit synaptics_clearpad_remove_i2c(struct i2c_client *i2c)
{
	struct synaptics_clearpad *this = dev_get_drvdata(&i2c->dev);
...
static const struct i2c_device_id synaptics_clearpad_i2c_id[] = {
	{ SYNAPTICS_CLEARPAD_NAME, 0 },
	{}
};
...
static struct i2c_driver synaptics_clearpad_i2c_driver = {
	.driver = {
		.name	= SYNAPTICS_CLEARPAD_NAME,
		.owner	= THIS_MODULE,
		.pm	= &synaptics_clearpad_pm,
	},
	.probe		= synaptics_clearpad_probe_i2c,
	.remove		= __devexit_p(synaptics_clearpad_remove_i2c),
	.id_table	= synaptics_clearpad_i2c_id,
};

static int __init synaptics_clearpad_init(void)
{
	int rc;
	rc = i2c_add_driver(&synaptics_clearpad_i2c_driver);
	return rc;
}

static void __exit synaptics_clearpad_exit(void)
{
	i2c_del_driver(&synaptics_clearpad_i2c_driver);
}

module_init(synaptics_clearpad_init);
module_exit(synaptics_clearpad_exit);

MODULE_DESCRIPTION(SYNAPTICS_CLEARPAD_NAME "ClearPad Driver");
